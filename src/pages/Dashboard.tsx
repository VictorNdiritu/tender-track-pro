...
import { FileText, Clock, CheckCircle2, XCircle, TrendingUp, DollarSign, Target, BarChart3 } from "lucide-react";
...
export default function Dashboard() {
  ...
  // Pipeline stats
  const pending = tenders.filter((e) => e.status === "pending_approval");
  const active = tenders.filter((e) => e.status === "approved" || e.status === "in_progress");
  const won = tenders.filter((e) => e.status === "completed");
  const lost = tenders.filter((e) => e.status === "rejected");
  ...
  const pipelineStats = [
    ...
    { label: "Total Pipeline", count: tenders.length, value: sumValue(tenders), icon: BarChart3, color: "text-foreground", bg: "bg-secondary" },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold" style={{ lineHeight: "1.1" }}>Bids Dashboard</h1>
      ...
      <Tabs defaultValue="pending">
        ...
        <TabsContent value="active">
          <EntryTable entries={active} onView={setSelectedEntry} loading={isLoading} />
        </TabsContent>
        <TabsContent value="won">
          <EntryTable entries={won} onView={setSelectedEntry} loading={isLoading} />
        </TabsContent>
        <TabsContent value="lost">
          <EntryTable entries={lost} onView={setSelectedEntry} loading={isLoading} />
        </TabsContent>
